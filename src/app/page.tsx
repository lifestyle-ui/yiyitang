...
export default async function DashboardPage() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto w-full">
      <div className="mb-6">
        <h1 className="text-xl font-bold" style={{ color: "#1A1A1A" }}>今日待辦</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <StatCard
          icon={<ClipboardList className="w-5 h-5" style={{ color: "#2C4A3E" }} />}
          label="今日任務" value={todayTasks.length} accent="#EFF4F1" />
        <StatCard
          icon={<AlertCircle className="w-5 h-5 text-red-500" />}
          label="逾期任務" value={overdueTasks} accent="#FEF2F2" />
        <StatCard
          icon={<Users className="w-5 h-5" style={{ color: "#2C4A3E" }} />}
          label="客戶總數" value={totalClients} accent="#EFF4F1" />
        <StatCard
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4" style={{ color: "#2C4A3E" }} />
                今日任務 ({todayOnly.length})
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              {todayOnly.length === 0 ? (
                <p className="px-5 py-8 text-center text-sm" style={{ color: "#A8A5A0" }}>今日沒有待辦任務 🎉</p>
              ) : (
        ...
        <div className="flex flex-col gap-4">
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Link href="/clients" className="text-xs hover:underline" style={{ color: "#2C4A3E" }}>查看全部</Link>
              </div>
            </CardHeader>
            <CardContent className="p-0">
              {recentClients.map((client) => (
                <Link key={client.id} href={`/clients/${client.id}`}
                  ...
                  <div>
                    <p className="text-sm font-medium" style={{ color: "#1A1A1A" }}>{client.name}</p>
                    <p className="text-xs" style={{ color: "#A8A5A0" }}>{formatDateTime(client.updatedAt)}</p>
                  </div>
                </Link>
              ))}
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <span className="text-sm" style={{ color: "#6A6560" }}>諮詢次數</span>
                <span className="font-semibold" style={{ color: "#1A1A1A" }}>{monthlyConsultations}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm" style={{ color: "#6A6560" }}>客戶總數</span>
                <span className="font-semibold" style={{ color: "#1A1A1A" }}>{totalClients}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm" style={{ color: "#6A6560" }}>處方待追蹤</span>
                <span className="font-semibold" style={{ color: expiringPrescriptions.length > 0 ? "#D97706" : "#1A1A1A" }}>
...
function StatCard({
  ...
  return (
    <div className="rounded-sm p-4" style={{ background: accent, border: `1px solid ${highlight ? "#F6D860" : "#ECEAE6"}` }}>
      <div className="mb-1">{icon}</div>
      <p className="text-2xl font-bold" style={{ color: highlight ? "#92400E" : "#1A1A1A" }}>{value}</p>
      <p className="text-xs mt-0.5" style={{ color: "#8A8580" }}>{label}</p>
    </div>
  );
}
        ...
        <li key={task.id} className="px-5 py-3 flex items-start gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <p className="text-sm font-medium truncate" style={{ color: "#1A1A1A" }}>{task.title}</p>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              {task.client && (
                <Link href={`/clients/${task.client.id}`} className="text-xs hover:underline" style={{ color: "#2C4A3E" }}>
                  {task.client.name}
                </Link>
              )}
              {task.dueDate && <span className="text-xs" style={{ color: "#A8A5A0" }}>{formatDate(task.dueDate)}</span>}
            </div>
          </div>
        </li>
      ))}

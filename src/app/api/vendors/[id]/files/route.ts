...
const prefix = (id: string) => `vendor/${id}`;

// List vendor files with signed URLs
export async function GET(_req: Request, { params }: Params) {
  const { id } = await params;
  const { data, error } = await supabase.storage.from(BUCKET).list(prefix(id), {
    limit: 200,
  ...
  const files = await Promise.all(
    (data || []).filter((f) => f.name).map(async (f) => {
      const { data: signed } = await supabase.storage
        .from(BUCKET)
        .createSignedUrl(`${prefix(id)}/${f.name}`, 60 * 60);
...
export async function POST(req: Request, { params }: Params) {
  ...
  for (const file of files) {
    ...
    const { error } = await supabase.storage
      .from(BUCKET)
      .upload(`${prefix(id)}/${safeName}`, Buffer.from(await file.arrayBuffer()), {
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  const { error } = await supabase.storage.from(BUCKET).move(`${prefix(id)}/${name}`, `${prefix(id)}/${target}`);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ ok: true, name: target });
}

// Delete a file: ?name=xxx
export async function DELETE(req: Request, { params }: Params) {
  ...
  const { error } = await supabase.storage.from(BUCKET).remove([`${prefix(id)}/${name}`]);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
